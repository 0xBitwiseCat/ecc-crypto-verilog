...
// ==========================================================
// Lógica de Precomputación: Identificación de Primos
// ==========================================================
bool is_prime(int n) {
    if (n <= 1) return false;
    for (int i = 2; i * i <= n; i++)
...
// Ensambla un punto compuesto sumando los primos precalculados
Point assemble_point_from_primes(int value, const std::map<int, Point>& precomp, const std::vector<int>& valid_primes) {
    Point Q = Point::infinity();
    int rem = value;
    while (rem > 0) {
        int largest_prime = 1;
        for (int p : valid_primes) {
            if (p <= rem) largest_prime = p;
            else break;
        }
        Q = point_add(Q, precomp.at(largest_prime));
        rem -= largest_prime;
    }
    return Q;
}
...
// ==========================================================
// Controlador Algoritmo: Ventana Deslizante (Prime-Based)
// ==========================================================
Point sliding_window_kP(uint32_t k, int w, const std::map<int, Point>& precomp, const std::vector<int>& valid_primes) {
    Point Q = Point::infinity();
    int i = 20; // 21 bits totales para cubrir hasta 2^20 (1,048,576)
    ...
    while (i >= 0) {
        if (((k >> i) & 1) == 0) {
            ...
            // Reemplazo de la lectura directa: Búsqueda o ensamblaje
            Point window_point = assemble_point_from_primes(value, precomp, valid_primes);
            Q = point_add(Q, window_point);
            i -= length; 
        }
    }
    return Q;
...
int main() {
    ...
    std::ofstream csv("performance_primes_results.csv");
    csv << "Window_Size,Precomp_Points,Memory_Bytes,Avg_Time_ns,Avg_Adds,Avg_Doubles,Avg_Total_Ops\n";
    
    std::cout << "========================================================================================\n";
    std::cout << " BENCHMARK: SLIDING WINDOW BASADO EN PRIMOS (w = 1 a 8) para k = 1..1048576\n";
    std::cout << "========================================================================================\n";
    ...
    for (int w = 1; w <= 8; ++w) {
        int max_window_val = (1 << w) - 1; 

        // 1. Filtrar los números válidos (1 + primos)
        std::vector<int> valid_primes = {1}; // 1 no es primo pero es estrictamente necesario como base
        for (int i = 2; i <= max_window_val; ++i) {
            if (is_prime(i)) valid_primes.push_back(i);
        }

        int num_precomp = valid_primes.size();
        int memory_bytes = num_precomp * 96;

        // 2. Fase de Precomputación (Deshabilitando contadores para no ensuciar el benchmark)
        hw_add_count = 0; hw_dbl_count = 0;
        std::map<int, Point> precomp;
        for (int p : valid_primes) {
            if (p == 1) { 
                precomp[1] = G; 
                continue; 
            }
            Point temp = Point::infinity();
            for (int c = 0; c < p; ++c) temp = point_add(temp, G);
            precomp[p] = temp;
        }

        // 3. Fase de Ejecución
        hw_add_count = 0; hw_dbl_count = 0; // Reiniciar para el benchmark
        auto start_time = std::chrono::high_resolution_clock::now();
        uint256_t dummy_sink = 0; 

        for (uint32_t k = 1; k <= MAX_K; ++k) {
            Point Q = sliding_window_kP(k, w, precomp, valid_primes);
            dummy_sink ^= Q.X; 
        }
